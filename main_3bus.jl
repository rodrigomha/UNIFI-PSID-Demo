...
sim = Simulation(ResidualModel, sys, pwd(), (0.0, 20.0), change_power_setpoint, all_lines_dynamic = true)
...
println("It looks like, eigenvalues for dynamic network 1-10 and 25-26 are clearly associated
...
scatter!(eigs_no_network, label = "Dynamic Network without fast eigvals")

println("Neglecting the network moves eigenvalues, but do not induce instability in this system")
...
plot(power, xlabel = "Time", ylabel = "Active Power [pu]", label = "Active Power Output GFM VSM Bus 3")

# Get Voltage Magnitude at Inverter Bus
voltage = get_voltage_magnitude_series(results, 103)
plot(voltage, xlabel = "Time", ylabel = "Voltage Magnitude [pu]", label = "Voltage Magnitude Bus 3")

voltage_bus2 = get_voltage_magnitude_series(results, 102)
plot(voltage_bus2, xlabel = "Time", ylabel = "Voltage Magnitude [pu]", label = "Voltage Magnitude Bus 2")